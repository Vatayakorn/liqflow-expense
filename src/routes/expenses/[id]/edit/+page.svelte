<script lang="ts">
    ...
    // Existing attachments (จาก database)
    let existingAttachments: Record<AttachmentType, Attachment[]> = $state({
        approve_proof:
            expense.attachments?.filter(
                (a: Attachment) => a.attachment_type === "approve_proof",
            ) ?? [],
        slip:
            expense.attachments?.filter(
                (a: Attachment) => a.attachment_type === "slip",
            ) ?? [],
        receipt:
            expense.attachments?.filter(
                (a: Attachment) => a.attachment_type === "receipt",
            ) ?? [],
        invoice:
            expense.attachments?.filter(
                (a: Attachment) => a.attachment_type === "invoice",
            ) ?? [],
        product_photo:
            expense.attachments?.filter(
                (a: Attachment) => a.attachment_type === "product_photo",
            ) ?? [],
    });

    // Attachments to delete
    let attachmentsToDelete = $state<string[]>([]);

    // ไฟล์แนบใหม่แต่ละประเภท (preview)
    ...
    function markForDeletion(attachment: Attachment) {
        attachmentsToDelete = [...attachmentsToDelete, attachment.id];
        existingAttachments[attachment.attachment_type as AttachmentType] =
            existingAttachments[
...
<svelte:head>
    <title>แก้ไขรายจ่าย | Liqflow Expense</title>
</svelte:head>
...
<div class="p-4 lg:p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">แก้ไขรายจ่าย</h1>
            <p class="text-sm text-gray-500 mt-0.5">แก้ไขข้อมูลรายจ่าย</p>
        </div>
    </div>
    ...
    <form
        ...
        use:enhance={({ formData }) => {
            ...
            // เพิ่มไฟล์ใหม่จาก state เข้าไปใน formData
                ...
                <div>
                    ...
                    <input
                        ...
                        value={form?.values?.date ??
                            toInputDate(new Date(expense.date))}
                ...
                <div>
                    ...
                    >
                        ...
                        {#each categories as cat}
                            <option
                                value={cat.id}
                                selected={form?.values?.category_id ===
                                    cat.id || expense.category_id === cat.id}
                            >
                                {cat.icon}
                                {cat.name}
                            </option>
            ...
            <div class="space-y-6">
                {#each ATTACHMENT_TYPES as attachType}
                    <div>
                        ...
                        {#if existingAttachments[attachType.value].length > 0}
                            <div class="flex flex-wrap gap-2 mb-2">
                                {#each existingAttachments[attachType.value] as attachment}
                                    <div class="relative group">
                                        {#if attachment.file_name?.match(/\.(jpg|jpeg|png|gif|webp)$/i)}
                                            <img
                                                src={getPublicUrl(
                                        ...
                                        <span
                                            class="absolute bottom-0 left-0 right-0 bg-green-500 text-white text-[10px] text-center py-0.5 rounded-b-lg"
                                        >
                                            บันทึกแล้ว
                                        </span>
                                    </div>
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <span class="text-sm text-gray-500">
                                {files[attachType.value].length} ไฟล์ใหม่
                            </span>
                        </div>
                                    ...
                                    <div class="relative group">
                                        {#if file.type.startsWith("image/")}
                                            <img
                                                ...
                                                alt={file.name}
                                                class="w-20 h-20 object-cover rounded-lg border-2 border-blue-400"
                                            />
                                        {:else}
                                            <div
                                                class="w-20 h-24 bg-gray-100 rounded-lg border-2 border-blue-400 flex flex-col items-center justify-center p-1"
                                            >
                                                <FileText
                                        ...
                                        <span
                                            class="absolute bottom-0 left-0 right-0 bg-blue-500 text-white text-[10px] text-center py-0.5 rounded-b-lg"
                                        >
                                            ใหม่
                                        </span>
                                    </div>
