<script lang="ts">
  ...
  onMount(async () => {
    ...
    const validateSession = async (s: Session | null) => {
      ...
      if (!email || !email.endsWith("@liqflow.com")) {
        await supabase.auth.signOut();
        goto("/login?error=invalid_domain");
        return;
      }
    ...
    const {
      ...
      await validateSession(_session);
    });
...
<div class="min-h-screen bg-gray-50">
  ...
  {:else}
    ...
    <aside
      class="fixed top-0 left-0 z-50 h-full w-64 bg-white border-r border-gray-200 transform transition-transform duration-200 ease-in-out lg:translate-x-0 {isSidebarOpen
        ? 'translate-x-0'
        : '-translate-x-full'}"
      style={darkMode
    ...
    >
      ...
      <!-- Navigation -->
      <nav class="p-4 space-y-1 flex-grow flex flex-col">
        ...
        <button
          ...
          class="w-full flex items-center gap-3 px-3 py-2 text-gray-500 rounded-lg transition-all hover:text-red-600 hover:bg-red-50 dark:text-gray-400 dark:hover:text-red-400 dark:hover:bg-red-900/10 mt-auto"
        >
